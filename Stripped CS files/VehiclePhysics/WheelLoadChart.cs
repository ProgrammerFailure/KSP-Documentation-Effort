...
public class WheelLoadChart : PerformanceChart
{
	...
	public override void SetupChannels()
	{
		...
		for (int num2 = m_wheels.Length; i < num2; i++)
		{
			...
			channel.scale = 1f / base.vehicle.cachedRigidbody.mass * 10f;
			channel.valueFormat = "0.0 Kg";
	...
	public override void RecordData()
	{
		...
		for (int num = m_wheels.Length; i < num; i++)
		{
			...
			{
				m_wheels[i].Write(wheelState.downforce * (1f / base.vehicle.gravity.Magnitude));
			}
		}
	}
}
