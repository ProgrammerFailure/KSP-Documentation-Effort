...
public class TextMeshPro : TMP_Text, ILayoutElement
{
	...
	public float m_previousLossyScaleY = -1f;
	...
	public int m_max_numberOfLines = 4;

	public Bounds m_default_bounds = new Bounds(Vector3.zero, new Vector3(1000f, 1000f, 0f));

	[SerializeField]
	public TMP_SubMesh[] m_subTextObjects = new TMP_SubMesh[8];
	...
	public override int SetArraySizes(int[] chars)
	{
		...
		for (int i = 0; i < chars.Length && chars[i] != 0; i++)
		{
			...
			if (glyph == null)
			{
				...
				{
					int spriteIndex = -1;
					tMP_SpriteAsset = TMP_SpriteAsset.SearchForSpriteByUnicode(tMP_SpriteAsset, num2, includeFallbacks: true, out spriteIndex);
					if (spriteIndex != -1)
					{
			...
			if (glyph == null)
			{
				...
				{
					int spriteIndex2 = -1;
					defaultSpriteAsset = TMP_SpriteAsset.SearchForSpriteByUnicode(defaultSpriteAsset, num2, includeFallbacks: true, out spriteIndex2);
					if (spriteIndex2 != -1)
					{
						m_textElementType = TMP_TextElementType.Sprite;
						m_textInfo.characterInfo[m_totalCharacterCount].elementType = m_textElementType;
			...
			if (glyph == null)
			{
				...
				if (glyph == null)
				{
					...
					chars[num4] = 32;
					num2 = 32;
					fontAssetForWeight = TMP_FontUtilities.SearchForGlyph(m_currentFontAsset, 32, out glyph);
		...
		for (int j = 0; j < num6; j++)
		{
			...
			{
				if (m_textInfo.meshInfo[j].vertices.Length - referenceCount * ((!m_isVolumetricText) ? 4 : 8) > 1024)
				{
			...
			else if (m_textInfo.meshInfo[j].vertices == null)
			{
				if (j == 0)
				{
	...
	public override void GenerateTextMesh()
	{
		if (!(m_fontAsset == null) && m_fontAsset.characterDictionary != null)
		{
			...
			if (m_char_buffer != null && m_char_buffer.Length != 0 && m_char_buffer[0] != 0)
			{
				...
				float num = (m_fontScale = m_fontSize / m_fontAsset.fontInfo.PointSize * m_fontAsset.fontInfo.Scale * (m_isOrthographic ? 1f : 0.1f));
				float num2 = num;
				...
				for (int i = 0; i < m_char_buffer.Length && m_char_buffer[i] != 0; i++)
				{
					...
					vector3.x = m_xAdvance + (m_cached_TextElement.xOffset - num5 - num6 + glyphValueRecord.xPlacement) * num2 * (1f - m_charWidthAdjDelta);
					vector3.y = (baseline + m_cached_TextElement.yOffset + num5 + glyphValueRecord.yPlacement) * num2 - m_lineOffset + m_baselineOffset;
					vector3.z = 0f;
					vector4.x = vector3.x;
					...
					{
						if ((num4 == 10 || char.IsSeparator((char)num4)) && num4 != 173 && num4 != 8203 && num4 != 8288)
						{
					...
					else if (!m_isRightToLeft)
					{
						...
						m_xAdvance += ((m_cached_TextElement.xAdvance * num34 * num7 + m_characterSpacing + m_currentFontAsset.normalSpacingOffset + glyphValueRecord.xAdvance) * num2 + m_cSpacing) * (1f - m_charWidthAdjDelta);
						if (char.IsWhiteSpace((char)num4) || num4 == 8203)
						{
							m_xAdvance += m_wordSpacing * num2;
						}
				...
				for (int j = 0; j < m_characterCount; j++)
				{
					...
					char character3 = characterInfo[j].character;
					...
					case TextAlignmentOptions.CaplineFlush:
					{
						...
						if (!((!char.IsControl(character4) && lineNumber < m_lineNumber) || flag12) && tMP_LineInfo.maxAdvance <= tMP_LineInfo.width)
						{
						...
						int num52 = tMP_LineInfo.visibleCharacterCount - 1 + tMP_LineInfo.controlCharacterCount;
						int num53 = (characterInfo[tMP_LineInfo.lastCharacterIndex].isVisible ? tMP_LineInfo.spaceCount : (tMP_LineInfo.spaceCount - 1)) - tMP_LineInfo.controlCharacterCount;
						if (flag10)
						{
							num53--;
							num52++;
					...
					{
						if ((flag11 || (j == 0 && (!char.IsPunctuation(character3) || char.IsWhiteSpace(character3) || character3 == '\u200b' || j == m_characterCount - 1))) && (j <= 0 || j >= characterInfo.Length - 1 || j >= m_characterCount || (character3 != '\'' && character3 != '’') || !char.IsLetterOrDigit(characterInfo[j - 1].character) || !char.IsLetterOrDigit(characterInfo[j + 1].character)))
						{
							num41 = ((j != m_characterCount - 1 || !char.IsLetterOrDigit(character3)) ? (j - 1) : j);
							flag11 = false;
					...
					if ((m_textInfo.characterInfo[j].style & FontStyles.Underline) == FontStyles.Underline)
					{
						...
						if (!flag && flag13 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character3 != '\n' && character3 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character3)))
						{
							flag = true;
					...
					if (num62)
					{
						...
						if (!flag2 && flag14 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character3 != '\n' && character3 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character3)))
						{
							flag2 = true;
					...
					if ((m_textInfo.characterInfo[j].style & FontStyles.Highlight) == FontStyles.Highlight)
					{
						...
						if (!flag3 && flag15 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character3 != '\n' && character3 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character3)))
						{
							flag3 = true;
