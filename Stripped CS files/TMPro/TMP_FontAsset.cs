...
public class TMP_FontAsset : TMP_Asset
{
	...
	public void ReadFontDefinition()
	{
		...
		for (int j = 0; j < kerningPairs.Count; j++)
		{
			...
			if (!m_kerningDictionary.ContainsKey((int)kerningPairKey.key))
			{
				m_kerningDictionary.Add((int)kerningPairKey.key, kerningPair);
			}
	...
	public bool HasCharacter(char character, bool searchFallbacks)
	{
		...
		if (searchFallbacks)
		{
			...
			if (TMP_Settings.fallbackFontAssets != null && TMP_Settings.fallbackFontAssets.Count > 0)
			{
				...
				{
					if (TMP_Settings.fallbackFontAssets[j].characterDictionary == null)
					{
						TMP_Settings.fallbackFontAssets[j].ReadFontDefinition();
					}
					if (TMP_Settings.fallbackFontAssets[j].characterDictionary != null && TMP_Settings.fallbackFontAssets[j].HasCharacter_Internal(character, searchFallbacks))
					{
						return true;
					}
				}
	...
	public bool HasCharacter_Internal(char character, bool searchFallbacks)
	{
		...
		if (searchFallbacks && fallbackFontAssets != null && fallbackFontAssets.Count > 0)
		{
			for (int i = 0; i < fallbackFontAssets.Count && fallbackFontAssets[i] != null; i++)
			{
				if (fallbackFontAssets[i].HasCharacter_Internal(character, searchFallbacks))
	...
	public bool HasCharacters(string text)
	{
		...
		while (true)
		{
			if (num < text.Length)
			{
				if (!m_characterDictionary.ContainsKey(text[num]))
				{
					break;
				}
				num++;
