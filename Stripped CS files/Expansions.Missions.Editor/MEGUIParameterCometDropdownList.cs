...
[MEGUI_CometSelect]
public class MEGUIParameterCometDropdownList : MEGUIParameter
{
	...
	public void SetDropdownValues()
	{
		...
		List<ActionCreateComet> allActionModules = MissionEditorLogic.Instance.EditorMission.GetAllActionModules<ActionCreateComet>();
		int i = 0;
		for (int count = allActionModules.Count; i < count; i++)
		{
			dropdownOptions.Add(allActionModules[i].PersistentID, allActionModules[i].comet.name);
			list.Add(allActionModules[i].comet.name);
		}
		if (list.Count == 0)
		{
