...
[MEGUI_FlagSelect]
public class MEGUIParameterFlagDropdownList : MEGUIParameter
{
	...
	public void SetDropdownValues()
	{
		...
		List<ActionCreateFlag> allActionModules = MissionEditorLogic.Instance.EditorMission.GetAllActionModules<ActionCreateFlag>();
		int i = 0;
		for (int count = allActionModules.Count; i < count; i++)
		{
			dropdownOptions.Add(allActionModules[i].persistentID, allActionModules[i].siteName);
			list.Add(allActionModules[i].siteName);
		}
		if (list.Count == 0)
		{
